...
class GenerateAIReviewUseCase:
    ...
    def execute(
        ...
        *,
        ...
        snapshot = build_ai_review_snapshot(
            as_of_date=as_of_date,
            timezone=timezone,
            ledger=ledger,
            progress=progress,
...
def _failure_code(failure: AIReviewProviderFailure) -> AIReviewFailureCode:
    return {
        ...
        AIReviewProviderFailure.AUTHENTICATION: AIReviewFailureCode.AUTHENTICATION,
        AIReviewProviderFailure.INVALID_REQUEST: AIReviewFailureCode.INVALID_REQUEST,
    }[failure]
